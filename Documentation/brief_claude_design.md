...
| Token | Hex | Uso |
|-------|-----|-----|
| `primary` | `#0F172A` | Encabezados, navbar, elementos de autoridad |
| `secondary` | `#334155` | Texto secundario, labels |
| `accent` | `#0369A1` | CTAs, links, estados activos |
| `background` | `#F8FAFC` | Fondo general |
| `foreground` | `#020617` | Texto principal |
| `card` | `#FFFFFF` | Tarjetas y formularios |
| `muted` | `#E8ECF1` | Fondos alternativos, separadores |
| `muted-foreground` | `#64748B` | Texto de ayuda, placeholders |
| `border` | `#E2E8F0` | Bordes de inputs y tarjetas |
| `destructive` | `#DC2626` | Acciones destructivas (anular, rechazar) |
| `success` | `#16A34A` | Confirmaciones, estados aprobados |
| `warning` | `#D97706` | Estados pendientes, alertas operativas |
...
- **Focus ring:** 3px solid `#0369A1` con offset de 2px. Siempre visible, nunca omitido.
...
```
[activo]          → fondo #DCFCE7, texto #166534, ícono check-circle
[anulado]         → fondo #FEE2E2, texto #991B1B, ícono ban
[desafiliado]     → fondo #F1F5F9, texto #475569, ícono log-out
[pendiente]       → fondo #FEF3C7, texto #92400E, ícono clock
[aprobada]        → fondo #DCFCE7, texto #166534, ícono check
[rechazada]       → fondo #FEE2E2, texto #991B1B, ícono x-circle
[cancelada]       → fondo #F1F5F9, texto #475569, ícono minus-circle
```

```
[adherente]       → fondo #EFF6FF, texto #1E40AF, ícono user
[afiliado]        → fondo #F0FDF4, texto #14532D, ícono badge-check
```

```
[validado]        → fondo #F0FDF4, texto #14532D, ícono shield-check
[no validado]     → fondo #FEF3C7, texto #92400E, ícono shield (outline)
```
...
- Error bajo el campo afectado, con ícono `alert-circle` rojo
...
| Variante | Uso | Estilo |
|----------|-----|--------|
| Primary | Acción principal de pantalla | `background: #0369A1`, texto blanco |
| Secondary | Acción secundaria | Borde `#0369A1`, texto `#0369A1`, fondo transparente |
| Destructive | Anular, rechazar | `background: #DC2626`, texto blanco |
| Ghost | Cancelar, volver | Sin borde, texto `#64748B` |
...
- Mensaje informativo (azul info): "Nombre obtenido del Registro Nacional. Puedes editarlo si hay un error."
- Botón primary: "Confirmar nombre"

**Caso B — Servicio DNI degradado/fallido:**
- Banner de aviso (amarillo, sin alarmar): "No pudimos verificar tu DNI automáticamente. Por favor ingresa tu nombre completo."
...
**Post-registro:**
- Pantalla de confirmación: ícono `check-circle` grande (verde), título "Cuenta creada", texto explicativo: "Tu cuenta ha sido registrada. Un operador revisará tus datos para validar tu identidad antes de continuar."
...
| `anulado` | Banner rojo: "Tu cuenta ha sido anulada. Contacta a un operador del partido." |
| `no validado` | Banner amarillo: "Tu identidad aún no ha sido validada por un operador. Este proceso es manual." |
...
- Splash screen con logo + color primary `#0F172A`
- `theme-color: #0F172A` en el manifest
...
- Focus ring siempre visible: 3px solid `#0369A1`, offset 2px
